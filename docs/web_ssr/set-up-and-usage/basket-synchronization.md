...
If the user logs in while Mealz is not active, you will need to call the /merge-authless-basket route to inform us of the login (see [Transferring an authless basket after login](/docs/web_ssr/set-up-and-usage/login-and-logout#transferring-an-authless-basket-after-login)). 
